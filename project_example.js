...
function createScene() {
  ...
  var heroMaterial = new THREE.MeshStandardMaterial({color:0x00ff00});
  ...
  // generate pig
  //var pig = generatePig();
  //pig.scale.set(0.5 * 0.5, 0.5 * 0.5, 0.5 * 0.5); // scale pig by half
  //scene.add(pig);
...
function generatePig() {
  ...
  //h.add(head.rotation, "z", 0.0, Math.PI, 0.01).name("Head Rotation");
  //h.add(body.rotation, "z", 0.0, Math.PI, 0.01).name("Body Rotation");
  //h.add(leg1.rotation, "z", 0.0, Math.PI, 0.01).name("leg1 Rotation");
  //h.add(pig.rotation, "z", 0.0, Math.PI, 0.01).name("Pig Rotation");
